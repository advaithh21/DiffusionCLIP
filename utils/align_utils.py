...
def run_alignment_direct(img, output_size):
    if not os.path.exists("pretrained/shape_predictor_68_face_landmarks.dat"):
        print('Downloading files for aligning face image...')
        os.system(f'wget -P pretrained/ http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2')
        os.system('bzip2 -dk pretrained/shape_predictor_68_face_landmarks.dat.bz2')
        print('Done.')
    predictor = dlib.shape_predictor("pretrained/shape_predictor_68_face_landmarks.dat")
    aligned_image = align_face_direct(img, predictor=predictor, output_size=output_size, transform_size=output_size)
    print("Aligned image has shape: {}".format(aligned_image.size))
    return aligned_image
...
def get_landmark_direct(img, predictor):
    """get landmark with dlib
	:return: np.array shape=(68, 2)
	"""
    detector = dlib.get_frontal_face_detector()
...
def align_face_direct(img, predictor, output_size=256, transform_size=256):
    """
	:param filepath: str
	:return: PIL Image
	"""

    lm = get_landmark_direct(img, predictor)
